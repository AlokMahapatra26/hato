...
interface StaticTweetCardProps {
  tweet: any;
}

export default function StaticTweetCard({ tweet }: StaticTweetCardProps) {
  const { data: session } = useSession();
  const [loading, setLoading] = useState(false);
  const [isLiked, setIsLiked] = useState(tweet.likes?.includes(session?.user?.id));
